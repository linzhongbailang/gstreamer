...
#define CROP_X 32
#define CROP_Y 24
#define OUTPUT_WIDTH 2688
#define OUTPUT_HEIGHT 1536
#define INPUT_WIDTH 2560
#define INPUT_HEIGHT 1440

static GstFlowReturn
prepare_output_drm_buffer(GstVideoMixer2 * mix, GstBuffer*inbuf, GstBuffer ** outbuf)
{
    gboolean mapped;
    GstMapInfo inmap, outmap;
    guint8 *pSrcBase, *pDstBase;
    guint8 *pSrc, *pDst;
	int h = 0;

    if (G_UNLIKELY(!mix->drmbufferpool))
    {
        guint outsize = OUTPUT_WIDTH * OUTPUT_HEIGHT * 3 / 2;
        mix->drmbufferpool = gst_drm_buffer_pool_new(GST_ELEMENT(mix), dce_get_fd(), mix->current_caps, outsize);
    }
    *outbuf = GST_BUFFER(gst_drm_buffer_pool_get(mix->drmbufferpool, FALSE));

    if(gst_buffer_map(inbuf, &inmap, GST_MAP_READ) && gst_buffer_map(*outbuf, &outmap, GST_MAP_READWRITE))
    {
        pSrcBase = inmap.data;
        pDstBase = outmap.data + CROP_Y * OUTPUT_WIDTH + CROP_X;

        for(h = 0; h < INPUT_HEIGHT; h++)
        {
            pSrc = pSrcBase + h * INPUT_WIDTH;
            pDst = pDstBase + h * OUTPUT_WIDTH;
            memcpy(pDst, pSrc, INPUT_WIDTH);
        }

        pSrcBase = inmap.data + INPUT_WIDTH * INPUT_HEIGHT;
        pDstBase = outmap.data + (OUTPUT_HEIGHT + CROP_Y/2) * OUTPUT_WIDTH + CROP_X;

        for (h = 0; h < INPUT_HEIGHT/2; h++)
        {
            pSrc = pSrcBase + h * INPUT_WIDTH;
            pDst = pDstBase + h * OUTPUT_WIDTH;
            memcpy(pDst, pSrc, INPUT_WIDTH);
...
#ifdef __linux__
  prepare_output_drm_buffer(mix, outbuf, &drmbuf);
  GST_VIDEO_MIXER2_UNLOCK(mix);
  if (drmbuf) {
      GST_LOG_OBJECT(mix,
          "Pushing buffer with ts %" GST_TIME_FORMAT " and duration %"
          GST_TIME_FORMAT, GST_TIME_ARGS(GST_BUFFER_TIMESTAMP(drmbuf)),
          GST_TIME_ARGS(GST_BUFFER_DURATION(drmbuf)));
      ret = gst_pad_push(mix->srcpad, drmbuf);
  }
#else
  GST_VIDEO_MIXER2_UNLOCK(mix);
  if (outbuf) {
...
#ifndef PACKAGE
#  define PACKAGE "gstvideomixer"
#  define GST_PACKAGE_NAME "gst-plugin-bad"
