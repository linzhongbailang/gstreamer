  ...
  while (TRUE) {
    ...
    switch (pres) {
      ...
      case GST_H264_PARSER_ERROR:
		...
		#else
		*skipsize = size - 3;
		GST_ERROR_OBJECT (h264parse, "Error parsing H.264 stream, Invalid H.264 stream, skip %d bytes", *skipsize);
	    goto skip;
		#endif
      case GST_H264_PARSER_NO_NAL:
	  	#if 0
		...
		#else		
        *skipsize = size - 3;
		GST_ERROR_OBJECT (h264parse, "Error parsing H.264 stream, No H.264 NAL unit found, skip %d bytes", *skipsize);
        goto skip;
		#endif
