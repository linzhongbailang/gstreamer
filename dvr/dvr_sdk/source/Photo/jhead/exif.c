...
static const TagTable_t TagTable[] = {
  ...
  { TAG_IMAGE_DESCRIPTION,		"ImageDescription", FMT_STRING, -1 },
...
int TagNameToValue(const char* tagName)
{
    ...
    for (i = 0; i < TAG_TABLE_SIZE; i++) {
        if (strcmp(TagTable[i].Desc, tagName) == 0) {
			ALOGE("found tag %s val %d\n", TagTable[i].Desc, TagTable[i].Tag);
            return TagTable[i].Tag;
        }
    }
	ALOGE("tag %s NOT FOUND", tagName);
    return -1;
}
...
//                        printf("\"\n");
        ...
        switch(Tag){
            ...
            case TAG_SUBSEC_TIME:
				strncpy(ImageInfo.SubSecTime, (char *)ValuePtr, sizeof(ImageInfo.SubSecTime) - 1);
				ImageInfo.SubSecTime[sizeof(ImageInfo.SubSecTime) - 1] = '\0';
                break;

            case TAG_SUBSEC_TIME_ORIG:
				strncpy(ImageInfo.SubSecTimeOrig, (char *)ValuePtr, sizeof(ImageInfo.SubSecTimeOrig) - 1);
				ImageInfo.SubSecTimeOrig[sizeof(ImageInfo.SubSecTimeOrig) - 1] = '\0';
                break;

            case TAG_SUBSEC_TIME_DIG:
				strncpy(ImageInfo.SubSecTimeDig, (char *)ValuePtr, sizeof(ImageInfo.SubSecTimeDig) - 1);
				ImageInfo.SubSecTimeDig[sizeof(ImageInfo.SubSecTimeDig) - 1] = '\0';
                break;

            case TAG_DATETIME_DIGITIZED:
				strncpy(ImageInfo.DigitizedTime, (char *)ValuePtr, sizeof(ImageInfo.DigitizedTime) - 1);
				ImageInfo.DigitizedTime[sizeof(ImageInfo.DigitizedTime) - 1] = '\0';
...

    {
        ...
        if (DIR_ENTRY_ADDR(DirStart, NumDirEntries) + 4 <= OffsetBase+ExifLength){
			ALOGE("DirStart %p offset from dirstart %d", DirStart, 2 + 12 * NumDirEntries);
            Offset = Get32u(DirStart+2+12*NumDirEntries);
            if (Offset){
                SubdirStart = OffsetBase + Offset;
                if (SubdirStart > OffsetBase+ExifLength || SubdirStart < OffsetBase){
					ALOGE("SubdirStart %p OffsetBase %p ExifLength %d Offset %d",
	...
	ALOGE("returning from ProcessExifDir");
}
...
#ifdef SUPERDEBUG
	ALOGE("Thumbnail %s end", (ImageInfo.ThumbnailAtEnd ? "at" : "NOT at"));
#endif
    if (DumpExifMap){
        unsigned a,b;
		ALOGE("Map: %05d- End of exif\n", length - 8);
...
static const TagTable_t* TagToTagTableEntry(unsigned short tag)
{
    ...
    for (i = 0; i < TAG_TABLE_SIZE; i++) {
        if (TagTable[i].Tag == tag) {
			ALOGE("found tag %d", tag);
            int format = TagTable[i].Format;
            if (format == 0) {
				ALOGE("tag %s format not defined ***** YOU MUST ADD THE FORMAT TO THE TagTable in exif.c!!!!", TagTable[i].Desc);
                return NULL;
            }
            return &TagTable[i];
        }
    }
	ALOGE("tag %d NOT FOUND", tag);
    return NULL;
}
...
static void writeExifTagAndData(int tag,
    ...
    if (format == FMT_STRING) {
        ...
        if (components <= 4) {
			...
			ALOGE("copying value %s to %d\n", (char*)value, (*DataWriteIndex));
        ...
        if (components <= 4) {
            ...
            Put32u(Buffer+(*DirIndex) + 8, (*DataWriteIndex)-8);   // Pointer
			ALOGE("copying %s to %d\n", (char*)value + sizeof(ExifAsciiPrefix), (*DataWriteIndex));
...
#ifdef SUPERDEBUG
			ALOGE("processing component %s format %s\n", curElement, formatStr(format));
#endif
...
static void create_EXIF_internal(ExifElement_t* elements, int exifTagCount, int gpsTagCount, int hasDateTimeTag, int hasFocalLengthTag, char* Buffer)
{
...
#ifdef SUPERDEBUG
    ALOGE("create_EXIF %d exif elements, %d gps elements\n", exifTagCount, gpsTagCount);
#endif
...

    {
...
#ifdef SUPERDEBUG
                ALOGE("create_EXIF saving tag %x value \"%s\"\n",elements[i].Tag, elements[i].Value);
#endif
		...
		ALOGE("Ending Exif section DirIndex = %d DataWriteIndex %d\n", DirIndex, DataWriteIndex);
    }
    ...
    if (gpsTagCount) {
        ...
        // End of directory - contains optional link to continued directory.
        Put32u(Buffer+DirIndex, 0);
		ALOGE("Ending GPS section DirIndex = %d DataWriteIndex %d", DirIndex, DataWriteIndex);
    }

    {
		...
		ALOGE("Starting Thumbnail section DirIndex = %d", DirIndex);
        DirIndex = DataWriteIndex;
        NumEntries = 2;
        if (hasFocalLengthTag) {
        ...
        // End of directory - contains optional link to continued directory.
        Put32u(Buffer+DirIndex, 0);
		ALOGE("Ending Thumbnail section DirIndex = %d DataWriteIndex %d", DirIndex, DataWriteIndex);
    }
