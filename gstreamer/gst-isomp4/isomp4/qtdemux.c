...
gst_qtdemux_init (GstQTDemux * qtdemux)
{
  ...
  qtdemux->canbuf = NULL;
  qtdemux->can_frames = 0;
  memset(&qtdemux->stBS, 0, sizeof(BitStream));
    ...
    guint64 position, guint64 byte_position)
{
...

  {
	  ...
	  guint32 can_frame_size;
	  guint32 sync_word;

	  memset(&v_can, 0, sizeof(Ofilm_Can_Data_T));
	  GetBuffer(&qtdemux->stBS, (uint8_t *)&sync_word, 4);
	  if (sync_word != 0xdeadbeef)
	  {
		  GST_ERROR("can buffer lost sync\n");
	  }
	  else
	  {
		  GetBuffer(&qtdemux->stBS, (uint8_t *)&can_frame_size, 4);
		  if (can_frame_size != sizeof(Ofilm_Can_Data_T))
		  {
			  GST_ERROR("can frame size mismatch, expect=%d, actual=%d\n", sizeof(Ofilm_Can_Data_T), can_frame_size);
			  SkipBuffer(&qtdemux->stBS, can_frame_size);
		  }
		  else
		  {
			  GetBuffer(&qtdemux->stBS, (uint8_t *)&v_can, can_frame_size);
		  }
	  }
	  
	  pCanMeta = gst_buffer_add_can_buf_meta(GST_BUFFER(buf), &v_can, sizeof(Ofilm_Can_Data_T));
	  if (!pCanMeta){
...
gst_qtdemux_loop_state_movie (GstQTDemux * qtdemux)
{
  ...
  guint32 frame_size_size = 4;
  guint32 sync_word_size = 4;
  guint32 size_size = 4;
  BitStreamSeek(&qtdemux->stBS, frame_size_size + stream->sample_index * (sync_word_size + size_size + sizeof(Ofilm_Can_Data_T)));
  GST_DEBUG("sample index: %u", stream->sample_index);
...
static void
qtdemux_tag_add_can(GstQTDemux * qtdemux, const char *tag1, const char *dummy,
GNode * node)
{
	GNode *data;
	int len;
...
#else
		qtdemux->canbuf = (guint8 *)malloc(len-16);
		memcpy(qtdemux->canbuf, (guint8 *)data->data + 16, len - 16);
		BitStreamAttach(&qtdemux->stBS, qtdemux->canbuf, len - 16);
		GetBuffer(&qtdemux->stBS, (uint8_t *)&qtdemux->can_frames, 4);
#endif
    }
  }
}
...
static const struct
{
  ...
  FOURCC_covr, GST_TAG_PREVIEW_IMAGE, NULL, qtdemux_tag_add_can }, {
  FOURCC__cvr, GST_TAG_IMAGE, NULL, qtdemux_tag_add_covr }, {
