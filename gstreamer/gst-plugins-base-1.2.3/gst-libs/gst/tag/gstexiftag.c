...
#define EXIF_THUMBNAIL_OFFSET	0x0201
#define EXIF_THUMBNAIL_LENGTH	0x0202
...
static const GstExifTagMatch tag_map_exif[] = {
  ...
  {GST_TAG_COMMENT, EXIF_THUMBNAIL_OFFSET, EXIF_TYPE_LONG, 0, NULL, NULL},
  {GST_TAG_COMMENT, EXIF_THUMBNAIL_LENGTH, EXIF_TYPE_LONG, 0, NULL, NULL},
  {NULL, 0, 0, 0, NULL, NULL}
};
    ...
    const GstExifTagMatch * tag_map)
{
  ...
  guint16 i;
  GstMapInfo info;
  guint thumbnail_offset, thumbnail_length;
  ...
  for (i = 0; i < entries; i++) {
	...
	if(tagdata.tag == EXIF_THUMBNAIL_OFFSET) //relative to tiff header
	{
		thumbnail_offset = tagdata.offset + 12;
		continue;
	}

	if(tagdata.tag == EXIF_THUMBNAIL_LENGTH)
	{
		thumbnail_length = tagdata.offset;
		
		char temp_value[16];
		g_snprintf(temp_value, sizeof(temp_value) / sizeof(char), "%d/%d", thumbnail_offset, thumbnail_length);
		gst_tag_list_add (exif_reader->taglist, GST_TAG_MERGE_REPLACE, tag_map[map_index].gst_tag, temp_value, NULL);
