...
my_segment_to_running_time (GstSegment * segment, GstClockTime val)
{
  ...
  if (GST_CLOCK_TIME_IS_VALID (val)) {
    res = ABS(gst_segment_to_running_time(segment, GST_FORMAT_TIME, val));
  }
  return res;
}
...
handle_gathered_gop (GstSplitMuxSink * splitmux)
{
  ...
  {
  	if(splitmux->reference_ctx->in_running_time != GST_CLOCK_STIME_NONE)
	{
    	queued_gop_time = splitmux->reference_ctx->in_running_time - splitmux->reference_ctx->out_running_time;
  	}
  }
  else
  {
  	if(splitmux->reference_ctx->in_running_time != GST_CLOCK_STIME_NONE &&
		splitmux->gop_start_time != GST_CLOCK_STIME_NONE)
  	{
    	queued_gop_time = splitmux->reference_ctx->in_running_time - splitmux->gop_start_time;
	}
  }
...
gst_splitmux_sink_change_state (GstElement * element, GstStateChange transition)
{
  ...
  switch (transition) {
    case GST_STATE_CHANGE_NULL_TO_READY:{
      ...
      set_next_filename(splitmux, NULL);
      break;
    }
    ...
    case GST_STATE_CHANGE_READY_TO_NULL:
      ...
      GST_SPLITMUX_BROADCAST_INPUT (splitmux);
      GST_SPLITMUX_BROADCAST_OUTPUT (splitmux);
	  g_queue_foreach (&splitmux->reference_ctx->queued_bufs, (GFunc) mq_stream_buf_free, NULL);
	  g_queue_clear (&splitmux->reference_ctx->queued_bufs);
	  g_queue_foreach (&splitmux->out_cmd_q, (GFunc) out_cmd_buf_free, NULL);
	  g_queue_clear (&splitmux->out_cmd_q);	  
      GST_SPLITMUX_UNLOCK (splitmux);
      break;
